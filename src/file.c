...
enum file_error_code write_file_lines_to_file(const char *filename, struct file *file) {
  ...
  /* check if line_len == 200 && buffer[line_len] != '\0' because it was an overflow */
  while(fgets(buffer, sizeof(buffer), f) != NULL) {
    ...
    /* FIXME: used memcpy because we know buffer and the line hold the same amount of chars and is correctly '\0' terminated.
     * stncpy wasn't happy because, even though we have line_len and know that the destination can hold exactly line_len chars (null term already persent),
     * strncpy kept warning about trunction happening from source buffer. memcpy works and is portable.
     *
    ...
    /* check if we are at the end of the allocation **lines region. if so, double size.
     * No worries because this is temp. we will shrink with realloc at the end.
     * 
     * TODO more intelligently do this. There is a chance we double the size of `lines`
     * at the end of the file if it all lines up prefectly.
     * */
  ...
  if (line_count > 0) {
    ...
    /* realistically, line will be null already here so this is slightly frivolous */
    free(file);
    file = NULL;
